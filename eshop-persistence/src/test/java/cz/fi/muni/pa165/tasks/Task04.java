...
public class Task04 extends AbstractTestNGSpringContextTests {
    ...
    public void leftJoinCagesWithPets() {
        ...
        List<ProductAndCategory> productAndCategory = em.createQuery("select new cz.fi.muni.pa165.tasks.ProductAndCategory(p, c) from"
                + " Product p left join p.categories c on c.name = 'Kitchen' order by p.name, c.name", ProductAndCategory.class).getResultList();
